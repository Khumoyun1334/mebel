    ...
    return (
      <div className="min-h-screen bg-lightBg flex flex-col items-center justify-center gap-4 md:gap-5 px-4 pt-[72px]">
        ...
        <p className="text-gray-500 text-sm md:text-base">
          Boshlash uchun chiroyli mebellarni qo'shing
        </p>
        <Link
          to="/products"
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-10">
        <h1 className="font-serif text-2xl md:text-4xl font-normal text-dark mb-6 md:mb-10">
          ...
          <span className="text-sm text-gray-500 font-sans">
            ({cart.length} {cart.length === 1 ? "mahsulot" : "mahsulot"})
          </span>
        </h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-10">
          <div className="flex-1 space-y-4">
            {cart.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-2xl p-4 md:p-6 flex flex-col sm:flex-row gap-4 shadow-sm"
              >
                <img
                  src={
                    item.img ||
                ...
                <div className="flex-1">
                  ...
                  <h3 className="font-serif text-base md:text-lg font-normal text-dark mb-2">
                    {item.name}
                  </h3>
                  <div className="flex items-center bg-cream rounded-full w-fit">
                    <button
                      onClick={() => updateQuantity(item.id, item.qty - 1)}
                      className="w-8 h-8 border-none bg-transparent cursor-pointer flex items-center justify-center rounded-l-full"
                    >
                      <FiMinus size={12} />
                    </button>
                    ...
                    <button
                      onClick={() => updateQuantity(item.id, item.qty + 1)}
                      className="w-8 h-8 border-none bg-transparent cursor-pointer flex items-center justify-center rounded-r-full"
                    >
                      <FiPlus size={12} />
                    </button>
                  </div>
                ...
                <div className="text-right flex sm:flex-col justify-between items-center sm:items-end gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      showToast("Buyum o'chirildi", "error");
                    }}
          ...
          <div className="lg:w-[380px]">
            <div className="bg-white rounded-2xl p-5 md:p-7 shadow-sm sticky top-24">
              <h2 className="font-serif text-xl md:text-2xl font-normal mb-5 md:mb-6 text-dark">
                Buyurtma Xulosasi
              </h2>
              ...
              {shipping > 0 && (
                <p className="text-center text-accent text-xs bg-accent-light py-2 rounded-lg mt-4">
                  ...
                  {formatPrice(freeShippingThreshold - total)} xarid qiling!
                </p>
              )}
              <button
              ...
              <button
                onClick={() => {
                  if (window.confirm("Savatni butunlay tozalamoqchimisiz?")) {
                    clearCart();
                    showToast("Savat tozalandi!", "success");
                  }
                }}
                className="w-full text-red-500 text-xs mt-3 hover:underline text-center block"
              >
                🗑️ Savatni tozalash
              </button>
              <Link
                to="/products"
                className="block text-center text-gray-500 text-xs mt-3 hover:text-accent transition-all"
              >
