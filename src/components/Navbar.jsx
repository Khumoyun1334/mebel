...
import image from "../assets/azi.png";
...
function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 flex items-center justify-between h-[72px] gap-2">
          ...
          <div className="hidden lg:flex items-center gap-1 md:gap-2">
            ...
            <button
              ...
              className={`bg-transparent border-none cursor-pointer p-2 flex items-center ${darkMode ? "text-gray-300" : "text-gray-500"}`}
            >
              <FiSearch size={18} />
            </button>
            ...
            <Link
              to="/wishlist"
              className={`bg-transparent border-none cursor-pointer p-2 flex relative items-center ${darkMode ? "text-gray-300" : "text-gray-500"}`}
            >
              <FiHeart size={20} />
              {wCount > 0 && (
            ...
            <Link
              to="/cart"
              className="bg-dark border-none cursor-pointer text-white px-3 md:px-5 py-1.5 md:py-2 rounded-full flex items-center gap-1 md:gap-2 text-[11px] md:text-xs font-semibold font-serif hover:bg-accent transition-all"
            >
          ...
          <div className="hidden lg:block border-t border-accent/15 py-3 px-4 bg-lightBg/98">
            <div className="max-w-7xl mx-auto">
              <form onSubmit={handleSearch} className="flex items-center gap-3">
                ...
                <button
                  type="submit"
                  className="bg-accent text-white px-6 py-3 rounded-full text-sm font-semibold hover:bg-accent-dark transition-all"
                >
                  Qidirish
                </button>
                <button
                  ...
                  className="text-gray-400 hover:text-dark p-2"
                >
                  <FiX size={20} />
                </button>
              </form>
              <div className="flex gap-3 mt-3 flex-wrap">
                ...
                <button
                  onClick={() => {
                    setSearchQ("yotoqxona");
                    setSearchOpen(false);
                    navigate("/products?q=yotoqxona");
                  }}
                  className="text-xs text-accent hover:underline"
                >
                  yotoqxona
          ...
          <div className="lg:hidden border-t border-accent/15 py-3 px-3 bg-lightBg/98">
            <form onSubmit={handleSearch} className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="flex-1 bg-accent text-white px-3 py-2 rounded-full text-sm font-semibold"
                >
                  Qidirish
                </button>
                <button
                  type="button"
                  onClick={() => setSearchOpen(false)}
                  className="border border-gray-300 text-gray-500 px-3 py-2 rounded-full text-sm"
                >
                  Bekor qilish
                </button>
              </div>
      ...
      {mobileOpen && (
        <div
          className={`fixed top-[72px] left-0 right-0 z-[999] flex flex-col ${darkMode ? "bg-dark" : "bg-white"} shadow-xl animate-slideDown max-h-[calc(100vh-72px)] overflow-y-auto`}
        >
          {navLinks.map((link) => (
            <Link
              key={link.path}
          ...
          <div
            className={`py-2 border-b ${darkMode ? "border-gray-800" : "border-gray-100"}`}
          >
            <Link
              to="/wishlist"
              onClick={() => setMobileOpen(false)}
